...
impl SegmentFetcher {
    ...
    pub async fn fetch_segment(&self, segment: &NzbSegment) -> SegmentFetchResult {
        ...
        for attempt in 0..=self.config.max_retries {
            ...
            match client.fetch_article(&segment.message_id).await {
                Ok(response) => {
                    debug!(
    ...
    pub async fn fetch_segments(&self, segments: &[NzbSegment]) -> Result<Vec<SegmentFetchResult>> {
        ...
        for segment in segments {
            let result = self.fetch_segment(segment).await;
...
mod tests {
    ...
    fn test_segment_fetch_result_clone() {
        ...
        assert_eq!(cloned.segment.number, result.segment.number);
