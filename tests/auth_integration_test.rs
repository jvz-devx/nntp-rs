...
async fn test_retry_after_authentication_failure() {
    ...
    // TODO: Implement when fault-injection server supports credential validation
    // and failed authentication responses. Currently, the fault-injection NNTP
    // server in .claude/skills/fault-injection-nntp-server/ accepts all credentials
    // without validation (see src/protocol.rs lines 128-132). To implement this test,
    // the server would need:
    // 1. Configurable credential acceptance (valid/invalid username/password pairs)
    // 2. Response code 481 for authentication failure
    // 3. Ability to remain unauthenticated after failed auth attempt
}
...
async fn test_auth_required_commands_blocked_when_not_authenticated() {
    // TODO: Implement when fault-injection server supports authentication enforcement
    // per command. Currently, the fault-injection NNTP server in
    // .claude/skills/fault-injection-nntp-server/ does not enforce authentication
    // requirements on commands (see src/protocol.rs). All commands are available
    // regardless of authentication state. To implement this test, the server needs:
    // 1. Configuration to mark commands as requiring authentication (e.g., POST, IHAVE)
    // 2. Authentication state checking before command execution
    // 3. Response code 480 when unauthenticated clients attempt auth-required commands
    //
    // RFC 4643 §2.4 specifies response code 480 for "authentication required"
}
...
async fn test_auth_required_commands_work_when_authenticated() {
    ...
    // This is blocked by the same limitation as test_auth_required_commands_blocked
    // (line 245): the fault-injection server doesn't enforce authentication
    // requirements per command, so we can't distinguish between commands that
    // work only when authenticated vs. commands available without authentication.
    // See .claude/skills/fault-injection-nntp-server/src/protocol.rs for details.
    //
    // Once the server supports auth-enforcement, implement tests for:
    // - POST command (RFC 3977 §6.3.1, often requires auth)
    // - IHAVE command (RFC 3977 §6.3.2, often requires auth)
    // - Selecting restricted newsgroups (server-dependent)
}
