...
mod live_pipelining_tests {
    ...
    async fn test_pipelining_single_article() {
        ...
        let (count, first, _last) = client.select_group(&group).await.unwrap();
        if count == 0 {
        ...
        let article_num = first.to_string();
    ...
    async fn test_pipelining_depth_2() {
        ...
        let ids: Vec<String> = (first..first + 2).map(|n| n.to_string()).collect();
        ...
        assert!(results[0].is_success(), "First response should be successful");
        assert!(results[1].is_success(), "Second response should be successful");
        assert!(!results[0].data.is_empty(), "First article should have data");
        assert!(!results[1].data.is_empty(), "Second article should have data");
    }
    ...
    async fn test_pipelining_depth_5() {
        ...
        let ids: Vec<String> = (first..first + 5).map(|n| n.to_string()).collect();
        let id_refs: Vec<&str> = ids.iter().map(|s| s.as_str()).collect();
    ...
    async fn test_pipelining_depth_10() {
        ...
        let ids: Vec<String> = (first..first + 10).map(|n| n.to_string()).collect();
        let id_refs: Vec<&str> = ids.iter().map(|s| s.as_str()).collect();
        let results = client
            .fetch_articles_pipelined(&id_refs, 10)
            .await
            .unwrap();
    ...
    async fn test_pipelining_partial_batch() {
        ...
        let ids: Vec<String> = (first..first + 7).map(|n| n.to_string()).collect();
        let id_refs: Vec<&str> = ids.iter().map(|s| s.as_str()).collect();
        let results = client.fetch_articles_pipelined(&id_refs, 5).await.unwrap();
