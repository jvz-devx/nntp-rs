...
fn test_parse_stat_response_with_article_number() {
    ...
    let info = commands::parse_stat_response(response).unwrap();
...
fn test_parse_stat_response_with_zero_article_number() {
    ...
    assert_eq!(info.message_id, "<xyz@test.com>");
}
...
fn test_parse_stat_response_rfc_example() {
    ...
    assert_eq!(info.message_id, "<message@example.com>");
}
...
fn test_parse_stat_response_complex_message_id() {
    ...
    assert_eq!(
        info.message_id,
        "<part1of10.yEnc.abc123@usenet.example.com>"
    );
}
...
fn test_parse_stat_response_very_large_article_number() {
    ...
    let info = commands::parse_stat_response(response).unwrap();
    assert_eq!(info.number, u64::MAX);
    assert_eq!(info.message_id, "<test@example.com>");
}
#[test]
...
fn test_parse_stat_response_message_id_with_spaces() {
    ...
    assert_eq!(info.message_id, "<test message@example.com>");
}
...
fn test_parse_stat_response_typical_binary_server() {
    ...
    let info = commands::parse_stat_response(response).unwrap();
    assert_eq!(info.number, 3456789);
    assert_eq!(
        info.message_id,
        "<20250121.part01.yEnc@provider.example.com>"
    );
}
