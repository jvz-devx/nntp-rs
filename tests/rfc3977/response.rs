...
#[test]
fn test_response_four_digit_code_parses_first_three() {
    // NOTE: This tests CLIENT-SIDE DEFENSIVE PARSING, beyond RFC 3977 requirements.
    // RFC 3977 §3.1 strictly requires exactly 3 digits for the status code.
    // Servers MUST NOT send 4-digit codes. However, our parser is lenient and
    // extracts the first 3 digits, treating the rest as part of the message.
    // This defensive behavior protects against malformed server responses.
    //
    // "2000 OK" - first 3 chars "200" are the code
    // Position 3 is "0", position 4 is " ", message starts at position 4
    // Per the parser: if line.len() > 4, message is line[4..] = " OK"
    let (code, msg) = parse_response_line("2000 OK").unwrap();
    assert_eq!(code, 200);
    // Message starts at position 4: " OK" (includes the space after "0")
    // Note: This is an edge case - RFC-compliant servers never send 4-digit codes
    assert_eq!(msg, " OK");
}
