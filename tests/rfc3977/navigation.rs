...
fn test_parse_next_response_basic() {
    ...
    let info = commands::parse_next_response(response).unwrap();
    assert_eq!(info.number, 12346);
    assert_eq!(info.message_id, "<next@example.com>");
}
...
fn test_parse_next_response_rfc_example() {
    ...
    let info = commands::parse_next_response(response).unwrap();
    assert_eq!(info.number, 1000235);
    assert_eq!(info.message_id, "<article.next@example.org>");
}
...
fn test_parse_next_response_complex_message_id() {
    ...
    let info = commands::parse_next_response(response).unwrap();
    assert_eq!(info.number, 500);
    assert_eq!(
        info.message_id,
        "<part2of10.yEnc.xyz789@binary.example.net>"
    );
}
...
fn test_parse_next_response_very_large_article_number() {
    ...
    let info = commands::parse_next_response(response).unwrap();
    assert_eq!(info.number, 18446744073709551614u64);
    assert_eq!(info.message_id, "<huge@example.com>");
}
...
fn test_parse_next_response_sequential_article() {
    ...
    assert_eq!(info.message_id, "<sequential@example.com>");
}
...
fn test_parse_next_response_sparse_articles() {
    ...
    let info = commands::parse_next_response(response).unwrap();
    assert_eq!(info.number, 1500);
    assert_eq!(info.message_id, "<sparse@example.com>");
}
#[test]
...
fn test_parse_next_response_message_id_with_spaces() {
    ...
    assert!(info.message_id.contains("test"));
    assert!(info.message_id.contains("message@example.com"));
}
...
fn test_next_response_binary_newsgroup_navigation() {
    ...
    let info = commands::parse_next_response(response).unwrap();
    assert_eq!(info.number, 9876543210);
    assert_eq!(
        info.message_id,
        "<binary.part.001of100@usenet.provider.net>"
    );
}
...
fn test_next_response_typical_server_response() {
    ...
    let info = commands::parse_next_response(response).unwrap();
    assert_eq!(info.number, 42001);
    assert_eq!(info.message_id, "<20250121120000.12345@news.example.com>");
}
...
fn test_next_sequential_navigation() {
    ...
    for (message, expected_num) in responses {
        ...
        let info = commands::parse_next_response(response).unwrap();
        assert_eq!(info.number, expected_num);
    }
}
#[test]
...
fn test_parse_last_response_basic() {
    ...
    let info = commands::parse_last_response(response).unwrap();
    assert_eq!(info.number, 12344);
    assert_eq!(info.message_id, "<prev@example.com>");
}
...
fn test_parse_last_response_rfc_example() {
    ...
    let info = commands::parse_last_response(response).unwrap();
    assert_eq!(info.number, 1000234);
    assert_eq!(info.message_id, "<article.prev@example.org>");
}
...
fn test_parse_last_response_complex_message_id() {
    ...
    let info = commands::parse_last_response(response).unwrap();
    assert_eq!(info.number, 999998);
    assert_eq!(info.message_id, "<part1of10.yEnc.6f3a2b1c@news.server.com>");
}
...
fn test_parse_last_response_very_large_article_number() {
    ...
    let info = commands::parse_last_response(response).unwrap();
    assert_eq!(info.number, u64::MAX - 1);
}
...
fn test_parse_last_response_sequential_navigation() {
    ...
    assert_eq!(info.message_id, "<msg100@example.com>");
}
...
fn test_last_response_binary_newsgroup_navigation() {
    ...
    let info = commands::parse_last_response(response).unwrap();
    assert_eq!(info.number, 4567899);
    assert!(info.message_id.contains("yEnc"));
}
...
fn test_last_response_typical_server_response() {
    ...
    let info = commands::parse_last_response(response).unwrap();
    assert_eq!(info.number, 41999);
    assert_eq!(info.message_id, "<20250121115959.12344@news.example.com>");
}
