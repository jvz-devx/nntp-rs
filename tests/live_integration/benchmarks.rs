...
async fn bench_article_retrieval_latency() {
    ...
    match result {
        Ok(stat_info) => {
            ...
            let start = Instant::now();
            let article = client.fetch_article(&stat_info.message_id).await;
            let elapsed = start.elapsed();
...
async fn bench_head_retrieval_latency() {
    ...
    match result {
        Ok(stat_info) => {
            ...
            let start = Instant::now();
            let headers = client.fetch_head(&stat_info.message_id).await;
            let elapsed = start.elapsed();
...
async fn bench_article_throughput() {
    ...
    for article_num in test_range {
        ...
        if let Ok(stat_info) = client.stat(&article_num.to_string()).await {
            match client.fetch_article(&stat_info.message_id).await {
                Ok(response) => {
                    success_count += 1;
